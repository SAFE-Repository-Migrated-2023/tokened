...
@if(isset($entry->id))
<div class="font-semibold text-2xl text-gray-700">{{ __('Edit job') }} for {{$contact->name}}</div>
@else
<div class="font-semibold text-2xl text-gray-700">{{ __('Add new job') }} for {{$contact->name}}</div>
@endif

<div class="rounded-xl shadow-md p-4 my-4">
<form method="POST" action="{{ isset($entry->id) ? route('jobs.update', $entry->id) : route('jobs.store') }}" autocomplete="off" enctype="multipart/form-data">
